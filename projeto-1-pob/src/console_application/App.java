		...
		switch(userInput) {
			...
			case "2":
				try {
					...
					Facade.addMoney(Double.parseDouble(value), loggedUser);
				} catch(Exception e) {
					System.out.println("\n" + e.getMessage());
				}
			...
			case "3":
				...
				System.out.println(String.format("Seu saldo: R$%s", userWallet));
				for(Game game : games) {
					System.out.println(String.format("Titulo: %s", game.getTitle()));
					System.out.println(String.format("Preço: R$%s", Facade.formatMoney(game.getPrice())));
					System.out.println();
